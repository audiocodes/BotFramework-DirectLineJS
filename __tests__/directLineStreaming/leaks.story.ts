...
jest.mock('../../src/streaming/WebSocketClientWithNetworkInformation', () => ({
  __esModule: true,
...
let mockFetchBody: Record<string, unknown> = { token: 'new-token' };
...
const mockFetch = jest.fn(async (url: string, options?: RequestInit) => {
  ...
  if (urlString === `${refreshEndpoint}${refreshPath}` && options?.method === 'POST') {
    refreshRequestCount += 1;
    return {
      ok: mockFetchStatus >= 200 && mockFetchStatus < 300,
      status: mockFetchStatus,
      statusText: mockFetchStatus === 403 ? 'Forbidden' : 'OK',
      json: async () => mockFetchBody
    } as Response;
  }

  // For other URLs, use original fetch if available
